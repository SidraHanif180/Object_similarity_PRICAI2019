function [net,stats] = cnn_train_dag_compare_UOL(net, imdb, getBatch, varargin)
...
if isempty(opts.train), opts.train = find(imdb.imagesA.set==1) ; end
if isempty(opts.val), opts.val = find(imdb.imagesA.set==3) ; end
...
for epoch=start+1:opts.numEpochs
  ...
  %% %%% Use previous net and previous imdb here
%   if epoch > start +1
%   previous_imdb = imdb;
%   imdb = imdb_train_oneFixed_half();
%   imdb = attach_proposals_onefixed(imdb);
  imdb = imdb_train_2012();
  imdb = attach_proposals_2012(imdb);
%   imdb = combine_imdb_previousNew(previous_imdb, imdb,...
%       state.batch_all, state.previouslossBB,state.previouslosssim);
%   
%   end
%   net.layers(90).block.allLosses = [];
%   net.layers(91).block.allLosses = [];
...
%   imdb = attach_proposals_onefixed(imdb);
  ...
  if numel(opts.gpus) <= 1
    [net, state, batch_all,previouslossBB,...
        previouslosssim] = processEpoch(net, state, params, 'train') ;
  ...
  state.batch_all = batch_all;
  state.previouslossBB = previouslossBB;
  state.previouslosssim = previouslosssim;
...
% -------------------------------------------------------------------------
function [net, state, batch_all, previouslossBB,...
    previouslosssim] = processEpoch(net, state, params, mode)
% -------------------------------------------------------------------------
...
for t=1:params.batchSize:numel(subset)
  ...
  for f = setdiff(fieldnames(stats)', {'num', 'time', 'previouslossBB',...
          'previouslosssim' })
    f = char(f) ;
...
% Save back to state.
previouslossBB = stats.previouslossBB;
previouslosssim = stats.previouslosssim;
stats = rmfield(stats, 'previouslossBB');
stats = rmfield(stats, 'previouslosssim');
...
for i = 1:numel(sel)
  ...
  if i==2
  stats.previouslossBB = net.layers(sel(i)).block.allLosses ;
  end
  if i==1
  stats.previouslosssim = net.layers(sel(i)).block.allLosses ;
  end
end
