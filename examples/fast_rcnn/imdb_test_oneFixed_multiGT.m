...
%% Image A
szImagePath = 'G:\Pascal_BBox3\Test\Images\';
bbPath = 'G:\Pascal_BBox3\Test\BBox\';
Image_Sizepath = 'G:\Pascal_BBox3\Test\Image_Size\';
proposal_path ='G:\Pascal_BBox3\Test\Proposals';
...
for ii = 1 : numel(dirList)
    ...
    propsAll{ii} = cell(f, 1);
    
    for jj = 1 : f %100 %numel(imgList)
        k = randperm (numel(imgList),1);
        imgsAll{ii}{jj} = fullfile(szImagePath, dirList(ii).name, imgList(k).name);
        
        inter_bbox =  load(fullfile(bbPath, bboxList(ii).name, valueList(k).name));
        bboxAll{ii}{jj} = inter_bbox.BBox';
        
        sizeImageInter = load(fullfile(Image_Sizepath, sizeList(ii).name, sizevalueList(k).name));
...
%     %rp = randperm(numel(imgsAll{ii}),30);
