function imdb = combine_proposals_saliency_T(imdb)
...
for i= 1: numel(imdb.imagesB.name)
    ...
    ind = min_box(single(pbox_all),single(gtbox_single));
%     ind = min_box(single(pbox_all),single(gtbb_allsin(m,:)));
    %end
%      
%     if isempty(intersect(imdb.bboxA.objlist{i},imdb.bboxB.objlist{i})),
%         ind(m)=randperm(6,1);
%     end
    targetpos =  pbox_all(ind,:);%imdb.bboxB.Imagesize{i}; 
%     reserve_ind = ind;
%     %[~, idx] = max(iou);    
