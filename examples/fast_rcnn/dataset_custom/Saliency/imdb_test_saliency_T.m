function imdb = imdb_test_saliency_T()
%% Image A
...
proposal_path =  fullfile(path_to_data, 'Salient_proposal');
...
for ii = 1 : numel(dirList)
    ...
    imgsAll{ii} = cell(floor(numel(imgList)/3), 1);
    
    valueList = dir(fullfile(bbPath, bboxList(ii).name, '*.mat'));
    bboxAll{ii} = cell(floor(numel(valueList)/3), 1);
   
    sizevalueList = dir(fullfile(Image_Sizepath, sizeList(ii).name, '*.mat'));
    sizeAll{ii} = cell(floor(numel(sizevalueList)/3), 1);
    ...
    objAll{ii} = cell(floor(numel(objList)/3), 1);
...
%     
    for jj = 1 : floor(numel(imgList)/3)            
        ...
        in = randperm(floor(numel(list_neg)/3),1);
