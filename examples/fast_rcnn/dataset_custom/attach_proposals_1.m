function imdb = attach_proposals_1(imdb)
...
%      pbox_all = imdb.bboxB.proposal{i};
    % pbox_all = imdb.bboxB.Imagesize{i};
...
%         ind(m)=randperm(6,1);
%     end
    targetpos = imdb.bboxB.Imagesize{i};% pbox_all(6,:); 
%     reserve_ind = ind;
%     %[~, idx] = max(iou);    
...
imdb.bboxB.ptarget = cell(numel(imdb.imagjesB.name),1);
for i= 1: numel(imdb.imagesB.name)    
